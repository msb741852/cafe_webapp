<?php
    ...
    while($row = mysqli_fetch_array($result)) {
        $list = $list."<li><a href=\"menu_Delete.php?id={$row['menuName']}\">{$row['menuName']}</a></li>";
    }
...
<head>
    ...
    <link rel="stylesheet" href="menu_Delete_Style.css?after">
...
<a href="https://blog.naver.com/ofete1588/220842438016" class="navbar_blog"><i class="fas fa-blog"></i></a>
    ...
    <h3 id="textCenter">어서오세요 ! 삭제하실 메뉴를 선택해주세요</h3>
    <section class="menuDelete">
        ...
        <div class="menuDelete_active">
            <span id="menuTitle" name='menuName'>선택하신 메뉴 : <?=$article['menuName']?></span>
            <form action="menu_Delete_update.php" method="POST">
                <input type="text" name='menuName' value='<?=$article['menuName']?>' hidden>
                <span  style="display: <?php if($article['menuName'] == ""){echo "none";}else{echo "block";}?>">정말로 삭제하시겠습니까?</span>
                <input style="display: <?php if($article['menuName'] == ""){echo "none";}else{echo "inline";}?>" id="secondinput" type="submit" value='삭제'>
            </form>
        </div>
    </section>
</body>
