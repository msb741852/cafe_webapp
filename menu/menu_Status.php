<?php
    $conn = mysqli_connect('localhost', 'root', 'Sb5623pt!', 'capston_opete');
    $sql = "SELECT * FROM menulist";
    $result = mysqli_query($conn, $sql);

    $list = '';
    while($row = mysqli_fetch_array($result)) {
        $list = $list."<li><a href=\"menu_Status.php?id={$row['menuName']}\">{$row['menuName']}</a></li>";
    }
...
<head>
    ...
    <link rel="stylesheet" href="menuStatusStyle.css?after">
...
<a href="https://blog.naver.com/ofete1588/220842438016" class="navbar_blog"><i class="fas fa-blog"></i></a>
    ...
    <h3 id="textCenter">어서오세요 ! 상태를 변경할 메뉴를 선택해주세요</h3>
    <section class="menuChange">
        <div class="menuChange_list">
            <span id="me"><a href="menu.php">메뉴선택</a></span>
            <ol id="menuList">
                <?=$list?>
            </ol>
        </div>
        ...
        <div class="menuChange_active">
            ...
            <form action="menu_Status_update.php" method="POST">
                <input type="text" name='menuName' value='<?=$article['menuName'] ?>' hidden>
                <select style="display: <?php if($article['menuName'] == ""){echo "none";}else{echo "block";}?>" name="menuStatus" id="sd" >
                    <option value="판매가능" <?php if($article['menuStatus'] == "판매가능") echo "SELECTED"; ?>>판매가능</option>
                    <option value="매진" <?php if($article['menuStatus'] == "매진") echo "SELECTED"; ?>>매진</option>
                </select>
